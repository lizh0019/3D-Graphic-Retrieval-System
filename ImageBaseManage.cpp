...
bool CImageBaseManage::CalculateAndSaveAllImageKnowledge()
{
	...
	//save the file
	CFile cWriteFile;
	if(cWriteFile.Open(sKnowledgeFullPathName,CFile::modeReadWrite|CFile::modeCreate)==false) return false;
...
//	FEATUREVECTOR stFeatureVector;
	...
	if(bRight==true)
	{
		this->CalculateSimilarity_Algorithm_2(pdSimilarityMatrix,pdFeature,iImageTotalNumber,iFeatureDimension);
	}
