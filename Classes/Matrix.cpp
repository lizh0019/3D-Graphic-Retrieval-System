...
bool CMatrix::LoadDataFromDoubleFile(CString sFullPathName)
{
	CFile cWriteFile;
	if(cWriteFile.Open(sFullPathName,CFile::modeReadWrite)==false) return false;
	double *pTemple=new double[m_nCol*m_nRow];
	if(cWriteFile.ReadHuge(pTemple,sizeof(double)*m_nCol*m_nRow)!=sizeof(double)*m_nCol*m_nRow) 
	{
		delete[]pTemple;
		cWriteFile.Close();
		return false;
