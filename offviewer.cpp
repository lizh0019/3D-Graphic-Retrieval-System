...
static int GLUTwindow_width = 400;
...
ReadOffFile(const char *filename)
{
  ...
  while (fgets(buffer, 1023, fp)) {
    ...
    if (nverts == 0) {
      ...
      Vertex& vert = mesh->verts[mesh->nverts++];       //haha
      ...
      // Get next face
      Face& face = mesh->faces[mesh->nfaces++];         //haha

      // Read number of vertices in face                //haha
      ...
      // Read vertex indices for face                   //haha
...
void GLUTRedrawtriangles(void)
{
  ...
  for (int i = 0; i < mesh->nfaces; i++) {
	...
	glBegin(GL_TRIANGLES);     //可以选择的10种模式
	//   	GL_POINTS     
	//   	GL_LINES 
	//  	GL_LINE_LOOP
	//      GL_LINE_STRIP      
...
void GLUTMousepolygon(int button, int state, int x, int y)
{
  // Invert y coordinate
  y = GLUTwindow_height - y;
  ...
  if (rotating || scaling || translating) glutIdleFunc(GLUTRedrawpolygon);      //original
  else glutIdleFunc(0);                                                         //original
...
void GLUTMousepoint(int button, int state, int x, int y)
{
  // Invert y coordinate
  y = GLUTwindow_height - y;
  ...
  scaling = (button == GLUT_MIDDLE_BUTTON);
  translating = (button == GLUT_RIGHT_BUTTON);
  if (rotating || scaling || translating) glutIdleFunc(GLUTRedrawpoint);      //
  else glutIdleFunc(0);                                                       //
...
void GLUTMouselines(int button, int state, int x, int y)
{
  // Invert y coordinate
  y = GLUTwindow_height - y;
  ...
  scaling = (button == GLUT_MIDDLE_BUTTON);
  translating = (button == GLUT_RIGHT_BUTTON);
  if (rotating || scaling || translating) glutIdleFunc(GLUTRedrawlines);      //
  else glutIdleFunc(0);                                                       //
...
void GLUTMousetriangles(int button, int state, int x, int y)
{
  // Invert y coordinate
  y = GLUTwindow_height - y;
  ...
  scaling = (button == GLUT_MIDDLE_BUTTON);
  translating = (button == GLUT_RIGHT_BUTTON);
  if (rotating || scaling || translating) glutIdleFunc(GLUTRedrawtriangles);      //
  else glutIdleFunc(0);                                                       //
...
void GLUTInitpolygon(int *argc, char **argv)
{
  // Open window 
//  glutInitpolygon(argc, argv);
  glutInitWindowPosition(300, 100);//initialize position of opengl window
  glutInitWindowSize(GLUTwindow_width, GLUTwindow_height);//size of opengl window
  glutInitDisplayMode(GLUT_DOUBLE | GLUT_RGB | GLUT_DEPTH); // | GLUT_STENCIL
  GLUTwindow = glutCreateWindow("OpenGL Viewer");

  // Initialize GLUT callback functions 
  glutReshapeFunc(GLUTResize);
  glutDisplayFunc(GLUTRedrawpolygon);
  glutKeyboardFunc(GLUTKeyboard);
  glutSpecialFunc(GLUTSpecial);
  glutMouseFunc(GLUTMousepolygon);
...
void GLUTInitpoint(int *argc, char **argv)
{
  // Open window 
//  glutInitpoint(argc, argv);
  ...
  // Initialize GLUT callback functions 
  glutReshapeFunc(GLUTResize);
  glutDisplayFunc(GLUTRedrawpoint);
  glutKeyboardFunc(GLUTKeyboard);
  glutSpecialFunc(GLUTSpecial);
  glutMouseFunc(GLUTMousepoint);
  glutMotionFunc(GLUTMotion);
  glutIdleFunc(0);
...
void GLUTInitlines(int *argc, char **argv)
{
  // Open window 
//  glutInitpoint(argc, argv);
  glutInitWindowPosition(300, 100);//initialize position of opengl window
  ...
  // Initialize GLUT callback functions 
  glutReshapeFunc(GLUTResize);
  glutDisplayFunc(GLUTRedrawlines);
  glutKeyboardFunc(GLUTKeyboard);
  glutSpecialFunc(GLUTSpecial);
  glutMouseFunc(GLUTMouselines);
  glutMotionFunc(GLUTMotion);
  glutIdleFunc(0);
...
void GLUTInittriangles(int *argc, char **argv)
{
  // Open window 
//  glutInitpoint(argc, argv);
  glutInitWindowPosition(300, 100);//initialize position of opengl window
  ...
  // Initialize GLUT callback functions 
  glutReshapeFunc(GLUTResize);
  glutDisplayFunc(GLUTRedrawtriangles);
  glutKeyboardFunc(GLUTKeyboard);
  glutSpecialFunc(GLUTSpecial);
  glutMouseFunc(GLUTMousetriangles);
  glutMotionFunc(GLUTMotion);
  glutIdleFunc(0);
